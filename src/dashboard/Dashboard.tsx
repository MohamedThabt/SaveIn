    ...
    return (
      <div className="h-screen flex flex-col bg-background text-foreground overflow-hidden font-sans scroll-smooth">
        ...
        <div className="flex-1 flex overflow-hidden w-full">
          {/* Settings Sidebar */}
          <aside className="w-[260px] border-r border-border/40 flex flex-col gap-6 shrink-0 hidden md:flex p-6 bg-card/30">
            <div className="px-2">
          ...
          {/* Settings Content */}
          <main className="flex-1 overflow-y-auto px-8 md:px-12 lg:px-16 py-8 pb-12 relative remove-scrollbar">
                  ...
                  <div className="bg-card border border-border/40 rounded-3xl p-6 md:p-8 shadow-sm flex flex-col gap-8">
                    ...
                    <div className="flex flex-col gap-5">
                      <div className="flex flex-col gap-2">
                        ...
                        <p className="text-[12px] text-muted-foreground">Create an internal integration in your <a href="https://www.notion.so/my-integrations" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline font-semibold">Notion Workspace</a>, then paste the secret token.</p>
                        <input type="password" value={notionToken} onChange={(e) => setNotionToken(e.target.value)} placeholder="secret_..." className="w-full px-5 py-3.5 border border-border/60 rounded-2xl text-[14px] bg-input text-foreground outline-none transition-all focus:border-primary focus:ring-4 focus:ring-primary/10 shadow-sm font-medium" />
                      </div>
                      <div className="flex flex-col gap-2">
                        <label className="text-[13px] font-bold text-foreground">2. Database Link or ID</label>
                        <p className="text-[12px] text-muted-foreground">Open your Notion database, click Share → Copy link, and paste it here.</p>
                        <input value={notionDb} onChange={(e) => setNotionDb(e.target.value)} placeholder="https://notion.so/workspace/abc123..." className="w-full px-5 py-3.5 border border-border/60 rounded-2xl text-[14px] bg-input text-foreground outline-none transition-all focus:border-primary focus:ring-4 focus:ring-primary/10 shadow-sm font-medium" />
                      </div>
                    </div>
    ...
    <div className="h-screen flex flex-col bg-background text-foreground overflow-hidden font-sans">
      ...
      <header className="flex items-center justify-between px-6 h-[72px] border-b border-border/50 shrink-0 bg-background/80 backdrop-blur-xl z-20 shadow-sm">
        ...
        <div className="flex-1 max-w-md mx-6 relative group">
          ...
          <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search posts, authors, tags..." className="w-full pl-9 pr-8 py-2.5 bg-input border border-border/40 rounded-xl text-[13px] font-medium text-foreground outline-none transition-all focus:border-primary/50 focus:ring-2 focus:ring-primary/10 shadow-sm placeholder:text-muted-foreground/60" />
          {searchQuery && (
                        ...
                        <div className="flex items-center justify-between mt-auto pt-4 border-t border-border/30">
                           <div className="flex items-center gap-2 overflow-hidden flex-1 pr-2">
                             {p.category && (
                              ...
                              {p.notionSyncStatus === 'synced' && (
                                <span className="inline-flex shrink-0 items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-green-50 text-green-600 dark:bg-green-950 dark:text-green-400 border border-green-200 dark:border-green-800" title="Synced to Notion">
                                  <Check size={9} /> Synced
                                </span>
                              )}
                           </div>
